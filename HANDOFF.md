...
- 此分支已設定 upstream：`origin/codex/home-restore-ui-20260717`。
- 本次 Git 流程文件會另以 `docs:` 提交；最新 Commit Hash 與 push 狀態應以 `git log -1`、`git status -sb` 及本次完成回報為準。
- 操作前仍應 Fetch 並確認遠端目前 Branch 是否出現其他提交，不沿用舊交接資訊推測同步狀態。
- `.backups/` 只留本機並由 `.gitignore` 排除；不要刪除或部署。
- 未 Merge、未 Production Deploy。
...
- 目前工作 Branch 已設定 upstream，可在 Sprint 驗證通過後直接 Commit 並 Push。
- 每次開始工作仍需以 `git status -sb` 與遠端 Fetch 結果確認是否同步。
- 尚未把目前分支整合回 `main`。
- Merge、Push 到 `main` 與 Production Deploy 仍需使用者明確確認。
...
3. 需要 Merge 或 Production Deploy 時先向使用者說明並取得確認。
...
Driver Pay Pro 的首頁、可設定平台收入模式及週報手機版已在分支 `codex/home-restore-ui-20260717` 完成；最新功能提交為 `4d4637b`。Sprint 驗證通過後，Codex 可直接 Commit 並 Push 目前工作 Branch，但不可自動 Merge 或 Deploy。首頁目前凍結；不要修改既有收入、計時、天氣、目標、歷史紀錄或統計邏輯。正式整合與 iPhone Safari／PWA 實機驗收仍待完成，開始任何修改前仍須先檢查實際 Git 狀態。
