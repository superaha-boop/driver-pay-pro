...
- Data：主要 KPI、金額、工時與目標值明顯逐級放大。`--font-data` 為
  24／27／30px，Driver 每日目標使用 `--font-input-value` 28／34／40px；Today
  主 KPI 與次 KPI 使用各自帶安全下限的 responsive token，不依賴一般數據 token。
- Controls：按鈕與一般輸入只適度放大，避免觸控元件因文字換行而破版。
- Structure：頁面／卡片標題、Driver 分類、Header、Logo、Bottom Navigation
  固定或只小幅變化；`--font-page-subtitle` 為 14／15／16px，顯示大小三個
  選項本身固定 14px。

實作 token 包含 `--font-kpi-primary`、`--font-kpi-secondary`、`--font-kpi-unit`、
`--font-data`、`--font-input-value`、`--font-button`、`--font-body`、
`--font-secondary`、`--font-section-title`、`--font-page-subtitle`、
`--font-navigation`，以及相容既有
