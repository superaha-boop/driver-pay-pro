...
- 原地展開整排至少 44px 並使用向下／向上 Chevron；進入下一層才使用向右
  Chevron，一般立即動作不使用箭頭。
- Today 收入摘要是本產品的例外 disclosure：收入、今日實際工時與目前時薪
  固定可見，整排只收合每日目標進度，不得為了精簡而隱藏三項核心資訊。
- Today 每日紀錄只顯示一個可操作日期卡；區塊標題與支出摘要不重複日期，支出
  付款日直接沿用該卡選定日期。支出金額為主要欄位，備註以同排次要入口展開。
- Driver 設定頁同一概念只顯示一次；標題、摘要、說明與狀態文字不得重複描述
  同一設定。Driver「常用」只保留每日目標與字體大小設定的直接控制；四個
  第一層名稱固定為「常用／收支／資料／系統」。
...
- 繼續跑車
- 收工
- 「今日工作狀態」整條標題列是現有工作明細的唯一展開／收合入口，至少
