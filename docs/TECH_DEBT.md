...
| TD-013 | Calendar has no date grid, session-selected date, or date deep link; it persists only `settings.calendarMonth`. | The target Overview → Locate → Open → Edit flow and fresh-session Today default are not yet implemented. | P1 | Calendar Implementation Sprint A | No; full state and route contract is now specified. |
...
| TD-018 | `loadState()` preserves damaged localStorage but falls back to default in-memory state without a user-visible read error. | Calendar can appear empty and mislead the user into thinking records are missing. | P1 | Calendar Implementation Sprint B before mutation release | No; read-error and retry behavior is specified. |
| TD-019 | Detail save and record deletion mutate in-memory state without transactional rollback when `saveState()` fails. | Calendar could display an unsaved edit/delete or lose a recoverable draft. | P1 | Calendar Implementation Sprint B | No; snapshot, rollback, retry, and SaveStatus are specified. |
...
| TD-021 | Existing tests do not cover a month grid, selected date, heat, future dates, Work Record Card, gestures, or Calendar accessibility. | A large Calendar replacement could regress date logic and interaction without detection. | P1 | Calendar Implementation Sprint A and B | No; required test matrix is now specified. |
