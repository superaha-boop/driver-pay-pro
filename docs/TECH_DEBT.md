...
| TD-002 | Service Worker cache names previously included long feature-specific version labels. | Manual release naming was error-prone and obscured which application revision was cached. | P2 | Resolved 2026-07-25 | Resolved; App Shell releases use the short sequential cache and current Production is `driver-pay-pro-v35`. Release automation remains a separate TD-005 concern. |
...
| TD-019 | Legacy Today writes can still mutate in-memory state before confirming durable storage. | A non-Calendar write can display a value that was not safely persisted. | P1 | Continue by approved Today mutation path | Partially resolved：Calendar、V1.1 work-time、明確 detail／expense／tip save、weather、shift 與 income record edit/delete 已採 transactional persistence。仍存在的其他 legacy `saveState()` 路徑需依各自核准 Sprint 稽核，不宣告整體完成。 |
...
| TD-029 | Installed PWA 從 iPhone 主畫面冷啟動／恢復後，日期 Picker 第一次操作仍可能未生效；Safari 分頁正常。 | 日期可能需要重複點擊或完全無法可靠開啟，妨礙歷史資料編輯；桌面模擬無法完整重現 iOS standalone native activation。 | P1 | 每次觸及 iOS 原生 input 或 PWA 發布時持續實機回歸 | Resolved by D-059 for current release：v37 移除 iOS 不可靠的 `showPicker()` 與 summary 強制 focus，改走 touchend 原生 input focus；Product Owner 多次冷啟動與 disclosure→日期交錯 Human QA 全部通過。長期裝置自動化限制仍由 TD-008 追蹤。 |
