...
- Driver 固定以「常用／收支／資料／系統」四個第一層分類呈現；「常用」預設
  展開，其餘預設收合。About 與系統狀態只在「系統」內，不建立獨立 About 頁
  或第二張狀態卡。
...
- Today 的「改用手動工時／改用開始／結束時間」是非破壞性的直接切換，不顯示
  ...
  已儲存工時；使用者實際修改目前模式欄位後，才沿用既有安全儲存流程。
...
- Driver 固定只有「常用／收支／資料／系統」四個第一層 disclosure。「常用」
  預設展開，其餘預設收合。設定頁遵循「同一概念只顯示一次」：每日目標只有
  一個標題與欄位，
  顯示偏好只有「字體大小設定」及標準／舒適／大字，不加入重複摘要或教學文案。
- Driver Header 摘要只在能幫助辨識內容時出現：「常用」不顯示摘要、「收支」
  顯示「平台、支出」、「資料」顯示「備份、匯出」、「系統」只顯示
  「正常」或「需要注意」。
- 「系統」只顯示可採取行動且能由目前執行環境可靠判定的狀態；不得堆疊紀錄
...
Service Worker 更新不得在使用者已可操作頁面後呼叫 `skipWaiting()`、
`clients.claim()` 或透過 `controllerchange` 強制重新載入。註冊可主動檢查並下載
新版本，但新 worker 必須等待既有頁面自然關閉，於下次自然啟動時生效，避免
吞掉第一次觸控或中斷尚未儲存的輸入。
