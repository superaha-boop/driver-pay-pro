...
test("Fixture 只存在測試目錄且不會進入 Production 或 App Shell", () => {
  ...
  assert.equal(fixture.expected.serviceWorkerCache, "driver-pay-pro-v16");
});
...
test("Service Worker v16 更新、清舊 cache 並保留 navigation fallback", () => {
  assert.match(serviceWorker, /const CACHE_NAME = "driver-pay-pro-v16"/);
