...
## Driver Settings Reliability and Usability — Human QA Candidate
...
- 四分類維持常用／收支／資料／系統；移除重複 Driver Space 頁首與模式摘要，
  統一低干擾回饋及 44px 移除控制。移除設定須確認，歷史資料與平台比例仍保留。
- storage key／WorkRecord／日期觸控／首頁 KPI typography／計算公式不變。
  App Shell v43，只升快取版本、不強制接管。完整備份還原仍待另行核准。
...
- Today 每日紀錄只顯示一個原生日期卡；標題為「每日紀錄」，支出摘要為
  「支出紀錄」，移除相同日期的重複文字與鎖定付款日期列。
- 支出仍由唯一 `activeRecordDate` 綁定付款日；Calendar、資料模型、計算與
  `driverPayApp.v2` 不變。
